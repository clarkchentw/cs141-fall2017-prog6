...
and below that the current total line length (0.5 pt)    |
 B.  5: Black points displayed according to first line of data file  |
                                               |
 ***The above features must work, otherwise no points given***       | 
                                                                     |
 C. 10: Add Point button pressed (only add points if pressed)        |
Each mouse click adds new blue point in that location    |
 D. 15: Draw Line button pressed (only draw lines after pressed)       |
...
Finally mouse released: Line end point is this location  |
 E.  5: Sum of line lengths shown in bottom-right                    |
 F.  5: Top message prints based on total length:                    |
  If length (so far) less than 105% of optimal: then indicate  |
  If length above: then indicate that instead                  |
 G. 10: Line ends for both beginning and end of line will click to   |
        the closest point.                                           |
